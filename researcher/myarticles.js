...
if (typeof(arxiv_authorid) == 'undefined') {
	...
	if (arxiv_authorid.indexOf('http://arxiv.org/a/')==0) {
	  arxiv_authorid=arxiv_authorid.substr(19,50);
	}
}
newScript.src='http://export.arxiv.org/fb/feed/'+arxiv_authorid+'/?format=json';
...
function makearXiv(feed)
{
	...
	for (x=0; x<num_entries; x++) {
		...
		for (format_name in feed.entries[x].formats[0]) {
			html += '<a href="' + feed.entries[x].formats[0][format_name] +'" title="Download '+format_name+'">'+ format_name+'</a>';
		}
